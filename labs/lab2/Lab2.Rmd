...
```{r}
billboard$rank = c(1:nrow(billboard))
```
...
*Hint: It might be helpful to look up what `str_detect` is doing!*

(Note the string '&' is different from the logical operator `&` which we will use later!!)

```{r}

billboard$colab = ifelse(str_detect(billboard$artist, '&'), 'Is colab', 'Is not colab')
