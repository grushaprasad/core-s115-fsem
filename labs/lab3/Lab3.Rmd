...
You can also rename existing columns using the `rename` function. For example the code below, renames the `Age` column to be `age`. 
...
<!-- The following code renames `Age` back to `age` -->
...
titanic_new = read.csv("titanic.csv") %>%
  ...
  mutate(age_in_decade = age/10,
         ...
         old = ifelse(age_decade_floor >= 6, "old", "not old"))
...
Write code to compute the mean age and standard error for the men who survived, women who survived, the men who did not survive, and the women who did not survive. Assign the resulting dataframe to a variable called `summ4`. 
